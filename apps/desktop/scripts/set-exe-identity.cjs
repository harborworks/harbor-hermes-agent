#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Harbor Works icon + version metadata onto the
// built app exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked app exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-Harbor-Works.exe>
//
...
// Stamp the Harbor Works icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Harbor Works',
      FileDescription: 'Harbor Works',
      CompanyName: 'Harbor Works',
      LegalCopyright: 'Copyright (c) 2026 Harbor Works'
    }
  })

  console.log('[set-exe-identity] done — Harbor Works icon + identity stamped')
}
